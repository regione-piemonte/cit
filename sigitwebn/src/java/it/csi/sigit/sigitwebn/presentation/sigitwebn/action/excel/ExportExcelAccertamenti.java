...
public class ExportExcelAccertamenti extends BaseAction<BaseSessionAwareDTO> {
	...
	public String execute() throws DbManagerException, SigitTIspezione2018DaoException {
		...
		try {
			ArrayList<Accertamento> ris= (ArrayList<Accertamento>) session.get("appDataelencoAccertamenti");
			for(Accertamento acc: ris){
				LOG.debug(acc.getCodiceImpianto());
			}
			LOG.debug("RIS: "+ ris);
			...
			String[] headers = {"identificativo","assegnato a","data creazione","data conclusione","stato","codice impianto","tipo anomalia","prov. competenza"};
			String[] fields = {"identificativo","assegnatario","dataCreazione","dataConclusione","descStatoAccertamento","codiceImpianto","descTipoAnomalia","siglaProv"};

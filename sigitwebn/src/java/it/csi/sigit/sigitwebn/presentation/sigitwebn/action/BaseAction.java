...
public abstract class BaseAction<T extends BaseSessionAwareDTO> extends ActionSupport
	...
	static {
		...
		MenuStructure menuRuoloMenu = new MenuStructure("menuRuolo");

		menuMap.add(menuRuoloMenu);

		MenuStructure menuAcquisisciMenu = new MenuStructure("menuAcquisisci");

		MenuStructure menuCodImpMenu = new MenuStructure("menuCodImp");

		menuAcquisisciMenu.getSubmenus().add(menuCodImpMenu);
		MenuStructure menuAcquistaBolliniMenu = new MenuStructure("menuAcquistaBollini");

		menuAcquisisciMenu.getSubmenus().add(menuAcquistaBolliniMenu);

		menuMap.add(menuAcquisisciMenu);

		MenuStructure menuAbilitazioniMenu = new MenuStructure("menuAbilitazioni");

		MenuStructure menuSubentroMenu = new MenuStructure("menuSubentro");

		menuAbilitazioniMenu.getSubmenus().add(menuSubentroMenu);
		MenuStructure menuDelegaMenu = new MenuStructure("menuDelega");

		menuAbilitazioniMenu.getSubmenus().add(menuDelegaMenu);
		MenuStructure menuIncaricoCatMenu = new MenuStructure("menuIncaricoCat");

		menuAbilitazioniMenu.getSubmenus().add(menuIncaricoCatMenu);
		MenuStructure menuTerzoRespMenu = new MenuStructure("menuTerzoResp");

		menuAbilitazioniMenu.getSubmenus().add(menuTerzoRespMenu);

		menuMap.add(menuAbilitazioniMenu);

		MenuStructure menuGestioneMenu = new MenuStructure("menuGestione");

		MenuStructure menuImpiantoMenu = new MenuStructure("menuImpianto");

		menuGestioneMenu.getSubmenus().add(menuImpiantoMenu);
		MenuStructure menuAllegatiMenu = new MenuStructure("menuAllegati");

		menuGestioneMenu.getSubmenus().add(menuAllegatiMenu);
		MenuStructure menuImportMassivoMenu = new MenuStructure("menuImportMassivo");

		menuGestioneMenu.getSubmenus().add(menuImportMassivoMenu);

		menuMap.add(menuGestioneMenu);

		MenuStructure menuPAMenu = new MenuStructure("menuPA");

		MenuStructure menuRicercaAvanzataImpMenu = new MenuStructure("menuRicercaAvanzataImp");

		menuPAMenu.getSubmenus().add(menuRicercaAvanzataImpMenu);
		MenuStructure menuDatiImpresaMenu = new MenuStructure("menuDatiImpresa");

		menuPAMenu.getSubmenus().add(menuDatiImpresaMenu);
		MenuStructure menuExportMassivoMenu = new MenuStructure("menuExportMassivo");

		menuPAMenu.getSubmenus().add(menuExportMassivoMenu);
		MenuStructure menuBackOfficeMenu = new MenuStructure("menuBackOffice");

		menuPAMenu.getSubmenus().add(menuBackOfficeMenu);
		MenuStructure menuRicercaVerificheMenu = new MenuStructure("menuRicercaVerifiche");

		menuPAMenu.getSubmenus().add(menuRicercaVerificheMenu);
		MenuStructure menuRicercaAccertamentiMenu = new MenuStructure("menuRicercaAccertamenti");

		menuPAMenu.getSubmenus().add(menuRicercaAccertamentiMenu);
		MenuStructure menuRicercaIspezioniMenu = new MenuStructure("menuRicercaIspezioni");

		menuPAMenu.getSubmenus().add(menuRicercaIspezioniMenu);
		MenuStructure menuRicercaSanzioniMenu = new MenuStructure("menuRicercaSanzioni");

		menuPAMenu.getSubmenus().add(menuRicercaSanzioniMenu);
		MenuStructure menuRicercaSveglieMenu = new MenuStructure("menuRicercaSveglie");

		menuPAMenu.getSubmenus().add(menuRicercaSveglieMenu);

		menuMap.add(menuPAMenu);

		MenuStructure menuDatiDistributoreMenu = new MenuStructure("menuDatiDistributore");

		menuMap.add(menuDatiDistributoreMenu);

		MenuStructure menuTokenMenu = new MenuStructure("menuToken");

		menuMap.add(menuTokenMenu);
	...
	private it.csi.sigit.sigitwebn.business.BackEndFacade backEnd = null;

	public void setSpringBackEnd(it.csi.sigit.sigitwebn.business.BackEndFacade be) {
		backEnd = be;
	}

	public it.csi.sigit.sigitwebn.business.BackEndFacade getSpringBackEnd() {
		return backEnd;
	}

	private it.csi.sigit.sigitwebn.business.SecurityHelper securityHelper = null;

	public void setSpringSecurityHelper(it.csi.sigit.sigitwebn.business.SecurityHelper sh) {
		securityHelper = sh;
	}

	public it.csi.sigit.sigitwebn.business.SecurityHelper getSpringSecurityHelper() {
	...
	public String getPortalCode() {
		return (String) session
				.get(it.csi.sigit.sigitwebn.presentation.multiportalutils.MultiPortalUtil.MDD_PORTAL_SESSIONATTR);
	}

	public String getChannelCode() {
		return (String) session
				.get(it.csi.sigit.sigitwebn.presentation.multiportalutils.MultiPortalUtil.MDD_CHANNEL_SESSIONATTR);
	}

	public String getStaticResource(String url) {
		return it.csi.sigit.sigitwebn.presentation.multiportalutils.MultiPortalUtil.getStaticResource(url,
				getPortalCode());
	}

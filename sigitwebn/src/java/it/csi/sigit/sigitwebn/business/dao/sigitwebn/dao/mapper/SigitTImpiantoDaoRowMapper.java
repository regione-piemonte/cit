...
public class SigitTImpiantoDaoRowMapper extends BaseDaoRowMapper implements org.springframework.jdbc.core.RowMapper {
	...
	public Object mapRow(ResultSet rs, int row) throws SQLException {
		...
		if (dtoInstance instanceof SigitTImpiantoAggiornamentoCoordinateDto) {
			return mapRow_internal((SigitTImpiantoAggiornamentoCoordinateDto) dtoInstance, rs, row);
		}
	...
	public SigitTImpiantoDto mapRow_internal(SigitTImpiantoDto objectToFill, ResultSet rs, int row)
		...
		if (mapAllColumns || columnsToReadMap.get("COORD_X_LONG_DD") != null)
			dto.setCoordXLongDd((Double) rs.getObject("COORD_X_LONG_DD"));
	...
	/**
	 * Metodo specifico di mapping relativo al DTO custom SigitTImpiantoAggiornamentoCoordinateDto.
	 * 
	 * @param rs
	 * @param row
	 * @throws SQLException
	 * @return SigitTImpiantoAggiornamentoCoordinateDto
	 * @generated
	 */

	public SigitTImpiantoAggiornamentoCoordinateDto mapRow_internal(
			SigitTImpiantoAggiornamentoCoordinateDto objectToFill, ResultSet rs, int row) throws SQLException {
		SigitTImpiantoAggiornamentoCoordinateDto dto = objectToFill;

		if (mapAllColumns || columnsToReadMap.get("CODICE_IMPIANTO") != null)
			dto.setImpiantiCodiceImpianto(rs.getBigDecimal("CODICE_IMPIANTO"));

		if (mapAllColumns || columnsToReadMap.get("ISTAT_COMUNE") != null)
			dto.setImpiantiIstatComune(rs.getString("ISTAT_COMUNE"));

		if (mapAllColumns || columnsToReadMap.get("DENOMINAZIONE_COMUNE") != null)
			dto.setImpiantiDenominazioneComune(rs.getString("DENOMINAZIONE_COMUNE"));

		if (mapAllColumns || columnsToReadMap.get("DENOMINAZIONE_PROVINCIA") != null)
			dto.setImpiantiDenominazioneProvincia(rs.getString("DENOMINAZIONE_PROVINCIA"));

		if (mapAllColumns || columnsToReadMap.get("SIGLA_PROVINCIA") != null)
			dto.setImpiantiSiglaProvincia(rs.getString("SIGLA_PROVINCIA"));

		if (mapAllColumns || columnsToReadMap.get("INDIRIZZO_SITAD") != null)
			dto.setImmobiliIndirizzoSitad(rs.getString("INDIRIZZO_SITAD"));

		if (mapAllColumns || columnsToReadMap.get("INDIRIZZO_NON_TROVATO") != null)
			dto.setImmobiliIndirizzoNonTrovato(rs.getString("INDIRIZZO_NON_TROVATO"));

		if (mapAllColumns || columnsToReadMap.get("CIVICO") != null)
			dto.setImmobiliCivico(rs.getString("CIVICO"));

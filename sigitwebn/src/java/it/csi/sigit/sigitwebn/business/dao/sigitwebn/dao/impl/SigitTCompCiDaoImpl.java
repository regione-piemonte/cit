...
/*PROTECTED REGION ID(R6582031) ENABLED START*/
...
public class SigitTCompCiDaoImpl extends AbstractDAO implements SigitTCompCiDao {
	...
	public void customDeleterByCodImpianto(Integer filter) throws SigitTCompCiDaoException {
		...
		/*PROTECTED REGION ID(R-1348267853) ENABLED START*/
	...
	public void customDeleterByCodImpiantoProgr(it.csi.sigit.sigitwebn.business.dao.sigitwebn.filter.CompFilter filter)
		...
		LOG.debug("[SigitTCompCiDaoImpl::customDeleterByCodImpiantoProgr] START");
		/*PROTECTED REGION ID(R515383009) ENABLED START*/
		//***scrivere la custom query
		...
		delete(jdbcTemplate, sql.toString(), params);
		LOG.debug("[SigitTCompCiDaoImpl::customDeleterByCodImpiantoProgr] END");
	}

	protected SigitTCompCiDaoRowMapper ByCodImpiantoRowMapper = new SigitTCompCiDaoRowMapper(null,
			SigitTCompCiDto.class, this);
	...
	public List<SigitTCompCiDto> findByCodImpianto(Integer input) throws SigitTCompCiDaoException {
		LOG.debug("[SigitTCompCiDaoImpl::findByCodImpianto] START");
		StringBuilder sql = new StringBuilder();
		MapSqlParameterSource paramMap = new MapSqlParameterSource();

		sql.append(
				"SELECT CODICE_IMPIANTO,ID_TIPO_COMPONENTE,PROGRESSIVO,DATA_INSTALL,LUNGHEZZA_CIRC_M,SUPERF_SCAMB_M2,PROF_INSTALL_M ");
		sql.append(" FROM SIGIT_T_COMP_CI");
		sql.append(" WHERE ");
		/*PROTECTED REGION ID(R-1059032479) ENABLED START*/
		sql.append("CODICE_IMPIANTO = :codImpianto");
		/*PROTECTED REGION END*/
		/*PROTECTED REGION ID(R-1884671999) ENABLED START*/
		paramMap.addValue("codImpianto", input, java.sql.Types.NUMERIC);
		/*PROTECTED REGION END*/
		List<SigitTCompCiDto> list = null;
		...
		try {
			...
			LOG.error("[SigitTCompCiDaoImpl::findByCodImpianto] esecuzione query", ex);
			throw new SigitTCompCiDaoException("Query failed", ex);
		} finally {
			stopWatch.dumpElapsed("SigitTCompCiDaoImpl", "findByCodImpianto", "esecuzione query", sql.toString());
			LOG.debug("[SigitTCompCiDaoImpl::findByCodImpianto] END");
		}
		return list;
	}

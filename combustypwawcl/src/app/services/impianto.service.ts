...
export class ImpiantoService {
  ...
  getGeoJsonByFilter(ricercaCompleta: boolean,
    ...
    if (ruolo != null) {
      parameters = parameters.append("ruolo", ruolo);
    }

    if (piva != null) {
      parameters = parameters.append("piva", piva);
    }

    if (codiceFiscalePF != null) {
      parameters = parameters.append("codiceFiscalePF", codiceFiscalePF);
    }
  ...
  parseGeoJsonToImpianto(geoJson: any): Impianto[] {
    ...
    return geoJson.features.map((feature: any) => {
      ...
      return {
        ...
        idPfResponsabile: properties['idPfResponsabile'] || '',
        idPgResponsabile: properties['idPgResponsabile'] || ''
      };
    });
  }

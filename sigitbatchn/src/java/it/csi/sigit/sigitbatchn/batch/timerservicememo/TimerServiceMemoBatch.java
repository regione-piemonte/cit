	...
	private void inviaSveglieUtentiNonAttivi() throws DbManagerException {
		...
		try {
			...
			if (!sveglieVerifiche.isEmpty()) {
				List<String> errors = sigitBatchMgr.invioSveglieUtentiNonAttivi("CIT: MEMO Sveglie VERIFICHE utenti non pi� attivi", "VERIFICA", sveglieVerifiche);
		...
		try {
			...
			if (!sveglieAccertamento.isEmpty()) {
				List<String> errors = sigitBatchMgr.invioSveglieUtentiNonAttivi("CIT: MEMO Sveglie ACCERTAMENTI utenti non pi� attivi", "ACCERTAMENTO", sveglieAccertamento);
				insertWrkLogMemo(errors);
			}
		} catch (Exception e) {
			insertWrkLogMemo(e);
		...
		try {
			...
			if (!sveglieIspezione2018.isEmpty()) {
				List<String> errors = sigitBatchMgr.invioSveglieUtentiNonAttivi("CIT: MEMO Sveglie ISPEZIONI utenti non pi� attivi", "ISPEZIONE", sveglieIspezione2018);
				insertWrkLogMemo(errors);
			}
		} catch (Exception e) {
			insertWrkLogMemo(e);
		...
		try {
			...
			if (!sveglieSanzioniAccertamenti.isEmpty()) {
				List<String> errors = sigitBatchMgr.invioSveglieUtentiNonAttivi("CIT: MEMO Sveglie SANZIONI (ACCERTAMENTI) di utenti non pi� attivi", "SANZIONE", sveglieSanzioniAccertamenti);
				insertWrkLogMemo(errors);
			}
		} catch (Exception e) {
			insertWrkLogMemo(e);
		...
		try {
			...
			if (!sveglieSanzioniIspezioni2018.isEmpty()) {
				List<String> errors = sigitBatchMgr.invioSveglieUtentiNonAttivi("CIT: MEMO Sveglie SANZIONI (ISPEZIONI) di utenti non pi� attivi", "SANZIONE", sveglieSanzioniIspezioni2018);
				insertWrkLogMemo(errors);
			}
		} catch (Exception e) {
			insertWrkLogMemo(e);

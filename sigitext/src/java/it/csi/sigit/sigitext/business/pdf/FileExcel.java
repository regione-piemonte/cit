...
public class FileExcel {
	...
	private DbServiceImp dbMgr;

	
	private ServiceManager serviziMgr;
	...
	public byte[] execute(List<Integer> ids)
	{
		...
		try
		{
			ArrayList<SigitTIspezione2018Dto> listIspezioni = new ArrayList<>();

			for (Integer id : ids) {
				SigitTIspezione2018Dto ispezione = getDbMgr().getSigitTIspezione2018Dao().findByPrimaryKey(new SigitTIspezione2018Pk(id));
				listIspezioni.add(ispezione);
			}
...
//			moduloBuilder.setActionPageBack(actionPageBack);
			...
			if (listIspezioni != null && listIspezioni.size() > 0)
			{
				...
				for (SigitTIspezione2018Dto ispezione2018 : listIspezioni) {
					
					if (ispezione2018 != null)
					{
						listAllIdIspezioni.add(ispezione2018.getIdIspezione2018().toString());
						
						if (ispezione2018.getCodiceImpianto() != null && ispezione2018.getCodiceImpianto().compareTo(BigDecimal.ZERO) > 0)
						{
							listCodImpianti.add(ispezione2018.getCodiceImpianto().toString());
						}
						else
						{
							listIdIspezioni.add(ispezione2018.getIdIspezione2018().toString());
						}
			...
			return null; // in alternativa array vuoto
		}
		catch (Exception ex)
		{
			log.error(ex);
	...
	private byte[] creaFileIspezioni(List<SigitExtIspezioniDto> listIspezioniAll, List<SigitExtIspezioniConCodImpiantoDto> listIspezioniConImpianto, List<SigitExtIspezioniSenzaCodImpiantoDto> listIspezioniSenzaImpianto) throws Exception
	{
		try {
			...
			*/
			...
			throw new Exception(Messages.ERROR_RECUPERO_SERVIZIO, e);
	...
	private void creaFoglioIspezioni(WritableWorkbook workbook, List<SigitExtIspezioniDto> listIspezioni) throws Exception
	{
		try {
			...
			if (listIspezioni != null)
			{
				for (int i = 0; i < listIspezioni.size(); i++) 
				{
					...
					valore = esitoBoolean == null ? "" : esitoBoolean ? Constants.DESC_ESITO_POSITIVO : Constants.DESC_ESITO_NEGATIVO;
	...
	private void creaFoglioConCodiceImpianto(WritableWorkbook workbook, List<SigitExtIspezioniConCodImpiantoDto> listIspezioniConImpianto) throws Exception
	{
		try {
	...
	private void creaFoglioSenzaCodiceImpianto(WritableWorkbook workbook, List<SigitExtIspezioniSenzaCodImpiantoDto> listIspezioniSenzaImpianto) throws Exception
	{
		try {
	...
	public DbServiceImp getDbMgr() {
		return dbMgr;
	}


	public void setDbMgr(DbServiceImp dbMgr) {
		this.dbMgr = dbMgr;
	}

	public ServiceManager getServiziMgr() {
		return serviziMgr;
	}


	public void setServiziMgr(ServiceManager serviziMgr) {

...
import it.csi.sigit.sigitext.business.dao.sigitextdao.dao.mapper.SigitVRicercaImpiantiDaoRowMapper;
...
import it.csi.sigit.sigitext.dto.sigitext.UnitaMisuraNox;
...
import static it.csi.sigit.sigitext.business.dao.util.Constants.ALLEGATO_TIPO_1;
import static it.csi.sigit.sigitext.business.dao.util.Constants.ALLEGATO_TIPO_1B;
...
public class SigitVRicercaImpiantiDaoImpl extends AbstractDAO implements SigitVRicercaImpiantiDao {
	...
	protected SigitVRicercaImpiantiDaoRowMapper byImpiantoFilterRowMapper = new SigitVRicercaImpiantiDaoRowMapper(new String[] { "CODICE_IMPIANTO", "ISTAT_COMUNE", "DENOMINAZIONE_COMUNE",
			"SIGLA_PROVINCIA", "DENOMINAZIONE_PROVINCIA", "FK_STATO", "L1_3_POT_H2O_KW", "L1_3_POT_CLIMA_INV_KW", "L1_3_POT_CLIMA_EST_KW", "FLG_NOPDR", "INDIRIZZO_UNITA_IMMOB", "CIVICO", "SEZIONE",
			"FOGLIO", "PARTICELLA", "SUBALTERNO", "POD_ELETTRICO", "PDR_GAS", "ID_PF_RESPONSABILE", "ID_PG_RESPONSABILE", "DENOMINAZIONE_RESPONSABILE", "CODICE_FISCALE", "DATA_FINE_PFPG", "RUOLO_RESPONSABILE", "RUOLO_FUNZ", "DES_STATO",
			"FLG_VISU_PROPRIETARIO" }, SigitVRicercaImpiantiDto.class, this);

	protected SigitVRicercaImpiantiDaoRowMapper ImpiantiDaRicercaAvanzataRowMapper = new SigitVRicercaImpiantiDaoRowMapper(null, SigitVRicercaImpiantiDto.class, this);
	...
	public List<SigitVRicercaImpiantiDto> findByImpiantoFilter(ImpiantoFilter input) throws SigitVRicercaImpiantiDaoException {
		LOG.debug("[SigitVRicercaImpiantiDaoImpl::findByImpiantoFilter] START");
		StringBuilder sql = new StringBuilder();
		MapSqlParameterSource paramMap = new MapSqlParameterSource();

		sql.append("SELECT DISTINCT CODICE_IMPIANTO,ISTAT_COMUNE,DENOMINAZIONE_COMUNE,SIGLA_PROVINCIA,DENOMINAZIONE_PROVINCIA,FK_STATO,L1_3_POT_H2O_KW,L1_3_POT_CLIMA_INV_KW,L1_3_POT_CLIMA_EST_KW,FLG_NOPDR,INDIRIZZO_UNITA_IMMOB,CIVICO,SEZIONE,FOGLIO,PARTICELLA,SUBALTERNO,POD_ELETTRICO,PDR_GAS,ID_PF_RESPONSABILE,ID_PG_RESPONSABILE,DENOMINAZIONE_RESPONSABILE,CODICE_FISCALE,DATA_FINE_PFPG,RUOLO_RESPONSABILE,RUOLO_FUNZ,DES_STATO,FLG_VISU_PROPRIETARIO ");
		...
		if (GenericUtil.isNotNullOrEmpty(input.getSiglaRea()) || GenericUtil.isNotNullOrEmpty(input.getNumeroRea()) || GenericUtil.isNotNullOrEmpty(input.getCfDitta())) {
		...
		try {
			...
			stopWatch.dumpElapsed("SigitVRicercaImpiantiDaoImpl", "findByImpiantoFilter", "esecuzione query", sql.toString());
	 ...
	 */
	@SuppressWarnings("unchecked")
	public List<SigitVRicercaImpiantiDto> findImpiantiDaRicercaAvanzata(RicercaAvanzataImpiantoFilter input) throws SigitVRicercaImpiantiDaoException {
		LOG.debug("[SigitVRicercaImpiantiDaoImpl::findImpiantiDaRicercaAvanzata] START");
		StringBuilder sql = new StringBuilder();
		MapSqlParameterSource paramMap = new MapSqlParameterSource();
		/*PROTECTED REGION ID(R1075669515) ENABLED START*/
		sql.append("SELECT DISTINCT I.CODICE_IMPIANTO,ISTAT_COMUNE,DENOMINAZIONE_COMUNE,SIGLA_PROVINCIA,DENOMINAZIONE_PROVINCIA,I.INDIRIZZO_UNITA_IMMOB,CIVICO,DENOMINAZIONE_RESPONSABILE,"
		...
		if (input.isRicercaAllegati()) {
			...
			if (GenericUtil.isNotNullOrEmpty(input.geteNoxDa()) || GenericUtil.isNotNullOrEmpty(input.geteNoxA())) {
				...
				if (ALLEGATO_TIPO_1.equals(input.getTipoRapporto()) || ALLEGATO_TIPO_1B.equals(input.getTipoRapporto())) {
		...
		if (input.isRicercaAllegati() && GenericUtil.isNotNullOrEmpty(input.getFlgVerificaAssociata()) && !ConvertUtil.convertToBooleanAllways(input.getFlgVerificaAssociata())) {
		...
		if (GenericUtil.isNotNullOrEmpty(input.getStatoImpianto())) {
			...
			paramMap.addValue("statoImp", ConvertUtil.convertToInteger(input.getStatoImpianto()), java.sql.Types.NUMERIC);
		}
		...
		if (GenericUtil.isNotNullOrEmpty(input.getFlgAppareccUiExt())) {
			...
			paramMap.addValue("flgAppExt", ConvertUtil.convertToBigDecimal(input.getFlgAppareccUiExt()), java.sql.Types.NUMERIC);
		}

		if (GenericUtil.isNotNullOrEmpty(input.getFlgContabilizzatore())) {

			sql.append(" AND I.FLG_CONTABILIZZAZIONE = :flgContab");
			paramMap.addValue("flgContab", ConvertUtil.convertToBigDecimal(input.getFlgContabilizzatore()), java.sql.Types.NUMERIC);
		}
		...
		if (GenericUtil.isNotNullOrEmpty(input.getIdTipoDocumento()) && !input.getIdTipoDocumento().toString().equals(Constants.ID_TUTTE)) {
		...
		if (GenericUtil.isNotNullOrEmpty(input.getSicurezzaImpianti())) {
			...
			paramMap.addValue("sicImpianti", ConvertUtil.convertToBigDecimal(input.getSicurezzaImpianti()), java.sql.Types.NUMERIC);
		}
		...
		if (GenericUtil.isNotNullOrEmpty(input.getNumGiorniInviatoDopoDataControllo())) {
			sql.append(" AND date_part('day', cast(DATA_INVIO as timestamp) - cast(DATA_CONTROLLO as timestamp)) > :numGiorniInviatoDopoDataControllo");
			paramMap.addValue("numGiorniInviatoDopoDataControllo", input.getNumGiorniInviatoDopoDataControllo(), java.sql.Types.NUMERIC);
		}

		log.debug("[SigitVRicercaImpiantiDaoImpl::findImpiantiDaRicercaAvanzata] - input.geteNoxDa(): " + input.geteNoxDa());
		if (GenericUtil.isNotNullOrEmpty(input.geteNoxDa())) {
			log.debug("[SigitVRicercaImpiantiDaoImpl::findImpiantiDaRicercaAvanzata] - input.getTipoRapporto(): " + input.getTipoRapporto());
			log.debug("[SigitVRicercaImpiantiDaoImpl::findImpiantiDaRicercaAvanzata] - input.getUnitaMisEnox(): " + input.getUnitaMisEnox());
			log.debug("[SigitVRicercaImpiantiDaoImpl::findImpiantiDaRicercaAvanzata] - UnitaMisuraNox.PPM_0: " + UnitaMisuraNox.PPM_0);
			log.debug("[SigitVRicercaImpiantiDaoImpl::findImpiantiDaRicercaAvanzata] - UnitaMisuraNox.MG_KWH_1: " + UnitaMisuraNox.MG_KWH_1);
			log.debug("[SigitVRicercaImpiantiDaoImpl::findImpiantiDaRicercaAvanzata] - UnitaMisuraNox.PPM_0.getCodice(): " + UnitaMisuraNox.PPM_0.getCodice());
			log.debug("[SigitVRicercaImpiantiDaoImpl::findImpiantiDaRicercaAvanzata] - UnitaMisuraNox.PPM_0.getDescrizione(): " + UnitaMisuraNox.PPM_0.getDescrizione());
		...
		log.debug("[SigitVRicercaImpiantiDaoImpl::findImpiantiDaRicercaAvanzata] - input.geteNoxA(): " + input.geteNoxA());
		...
		sql.append(" LIMIT (SELECT VALORE_CONFIG_NUM FROM SIGIT_WRK_CONFIG WHERE CHIAVE_CONFIG='MAX_RIGHE_RIC_AVZ_IMP') ");
		/*PROTECTED REGION END*/
		List<SigitVRicercaImpiantiDto> list = null;
		StopWatch stopWatch = new StopWatch(Constants.APPLICATION_CODE);
		try {
			...
			stopWatch.dumpElapsed("SigitVRicercaImpiantiDaoImpl", "findImpiantiDaRicercaAvanzata", "esecuzione query", sql.toString());

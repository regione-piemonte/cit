...
public class SigitVRicercaAllegatiDaoImpl extends AbstractDAO implements SigitVRicercaAllegatiDao {
	...
	protected SigitVRicercaAllegatiDaoRowMapper findBySiglaReeNumeroReeRowMApper = new SigitVRicercaAllegatiDaoRowMapper(null, SigitVRicercaAllegatiDto.class, this);
	...
	public List<SigitVRicercaAllegatiDto> findByCodImpianto(Integer input) throws SigitVRicercaAllegatiDaoException {
		...
		sql.append(" OR FK_TIPO_DOCUMENTO IN (" + Constants.MANUTENZIONE_GT + "," + Constants.MANUTENZIONE_GF + "," + Constants.MANUTENZIONE_SC + "," + Constants.MANUTENZIONE_CG + "," + Constants.RAPPORTO_PROVA_GT + "," + Constants.RAPPORTO_PROVA_GF +")) ");
	...
	public List<SigitVRicercaAllegatiDto> findBySiglaReeNumeroRee(String siglaRee, Long numeroRee) throws SigitVRicercaAllegatiDaoException {
		LOG.debug("[SigitVRicercaAllegatiDaoImpl::findBySiglaReeNumeroRee] START");
		StringBuilder sql = new StringBuilder();
		MapSqlParameterSource paramMap = new MapSqlParameterSource();
		sql.append("SELECT ID_ALLEGATO,FK_STATO_RAPP,FK_ISPEZ_ISPET,FK_TIPO_DOCUMENTO,FK_SIGLA_BOLLINO,FK_NUMERO_BOLLINO,DATA_CONTROLLO,B_FLG_LIBRETTO_USO,B_FLG_DICHIAR_CONFORM,B_FLG_LIB_IMP,B_FLG_LIB_COMPL,F_OSSERVAZIONI,F_RACCOMANDAZIONI,F_PRESCRIZIONI,F_FLG_PUO_FUNZIONARE,F_INTERVENTO_ENTRO,F_ORA_ARRIVO,F_ORA_PARTENZA,F_DENOMINAZ_TECNICO,F_FLG_FIRMA_TECNICO,F_FLG_FIRMA_RESPONSABILE,DATA_INVIO,DATA_RESPINTA,NOME_ALLEGATO,DATA_ULT_MOD,UTENTE_ULT_MOD,DES_RUOLO,RUOLO_FUNZ,ID_PERSONA_GIURIDICA,PG_DENOMINAZIONE,PG_CODICE_FISCALE,PG_SIGLA_REA,PG_NUMERO_REA,CODICE_IMPIANTO,COMUNE_IMPIANTO,SIGLA_PROV_IMPIANTO,INDIRIZZO_UNITA_IMMOB,CIVICO_UNITA_IMMOB,DES_TIPO_DOCUMENTO,DES_STATO_RAPP,FLG_CONTROLLO_BOZZA,UID_INDEX,UID_INDEX_FIRMATO,NOME_ALLEGATO_FIRMATO,ELENCO_COMBUSTIBILI,ELENCO_APPARECCHIATURE,FK_PG_CAT,PG_FK_STATO_PG,ID_TIPO_MANUTENZIONE,DES_TIPO_MANUTENZIONE,ALTRO_DESCR,UID_INDEX_FIRMATO,NOME_ALLEGATO_FIRMATO ");
		sql.append(" FROM VISTA_RICERCA_ALLEGATI");
		sql.append(" WHERE ");
		sql.append("FK_SIGLA_BOLLINO = :fkSiglaBollini ");
		sql.append(" AND FK_NUMERO_BOLLINO = :fkNumeroBollini ");
		sql.append(" AND FK_STATO_RAPP = 1 ");
		sql.append(" AND FK_TIPO_DOCUMENTO in (3,4,5,6) ");
		paramMap.addValue("fkSiglaBollini", siglaRee);
		paramMap.addValue("fkNumeroBollini", numeroRee);
		List<SigitVRicercaAllegatiDto> list = null;
		StopWatch stopWatch = new StopWatch(Constants.APPLICATION_CODE);
		try {
			stopWatch.start();
			list = jdbcTemplate.query(sql.toString(), paramMap, findBySiglaReeNumeroReeRowMApper);

		} catch (RuntimeException ex) {
			LOG.error("[SigitVRicercaAllegatiDaoImpl::findBySiglaReeNumeroRee] esecuzione query", ex);
			throw new SigitVRicercaAllegatiDaoException("Query failed", ex);
		} finally {
			stopWatch.dumpElapsed("SigitVRicercaAllegatiDaoImpl", "findBySiglaReeNumeroRee", "esecuzione query", sql.toString());
			LOG.debug("[SigitVRicercaAllegatiDaoImpl::findBySiglaReeNumeroRee] END");
		}
		return list;
	}
	
	public List<String> findAllSiglaRee() throws SigitVRicercaAllegatiDaoException {
		LOG.debug("[SigitVRicercaAllegatiDaoImpl::findAllSiglaRee] START");
		StringBuilder sql = new StringBuilder();
		MapSqlParameterSource paramMap = new MapSqlParameterSource();
		sql.append("select distinct SIGLA_BOLLINO ");
		...
		try {
			stopWatch.start();
			list = jdbcTemplate.query(sql.toString(), paramMap, new RowMapper<String>() {

				@Override
				public String mapRow(ResultSet rs, int arg1) throws SQLException {					
					return rs.getString("SIGLA_BOLLINO");
				}
			});

		} catch (RuntimeException ex) {
			LOG.error("[SigitVRicercaAllegatiDaoImpl::findAllSiglaRee] esecuzione query", ex);
			throw new SigitVRicercaAllegatiDaoException("Query failed", ex);
		} finally {
			stopWatch.dumpElapsed("SigitVRicercaAllegatiDaoImpl", "findAllSiglaRee", "esecuzione query", sql.toString());
			LOG.debug("[SigitVRicercaAllegatiDaoImpl::findAllSiglaRee] END");
		}
		return list;
	}		
	...
	public List<SigitVRicercaAllegatiDto> findRapportiProvaByCodiceImpianto(Integer codImpianto) throws SigitVRicercaAllegatiDaoException {
		LOG.debug("[SigitVRicercaAllegatiDaoImpl::findRapportiProvaByCodiceImpianto] START");
		StringBuilder sql = new StringBuilder();
		MapSqlParameterSource paramMap = new MapSqlParameterSource();
		...
		sql.append(" WHERE ");
		sql.append(" CODICE_IMPIANTO = :codImpianto");
		sql.append(" AND FK_TIPO_DOCUMENTO IN (" + Constants.RAPPORTO_PROVA_GF + "," + Constants.RAPPORTO_PROVA_GT + ") ");
		
		paramMap.addValue("codImpianto", codImpianto);
		...
		try {
			...
			LOG.error("[SigitVRicercaAllegatiDaoImpl::findRapportiProvaByCodiceImpianto] esecuzione query", ex);
			throw new SigitVRicercaAllegatiDaoException("Query failed", ex);
		} finally {
			stopWatch.dumpElapsed("SigitVRicercaAllegatiDaoImpl", "findRapportiProvaByCodiceImpianto", "esecuzione query", sql.toString());
			LOG.debug("[SigitVRicercaAllegatiDaoImpl::findRapportiProvaByCodiceImpianto] END");
		}
	...
	@Override
	public boolean checkAllegatoInviatoBetweenTwoDatesAndCodImpAndFkStatoRapp(Date dataContrattoFrom, Date dataContrattoTo, String codiceImpianto) {
		...
		String sql = 
				...
				+ " FROM " + getTableName()
				+ " where codice_impianto =:codiceImpianto"
				+ " AND fk_stato_rapp =:fkStatoRapp"
				+ " AND data_controllo BETWEEN :dataControlloFrom AND :dataControlloTo";

		Map<String, Object> params = new HashMap<>();
		params.put("codiceImpianto", Integer.valueOf(codiceImpianto));
		params.put("fkStatoRapp", Constants.ID_STATO_RAPPORTO_INVIATO);
		params.put("dataControlloFrom", new java.sql.Date(dataContrattoFrom.getTime()));
		params.put("dataControlloTo", new java.sql.Date(dataContrattoTo.getTime()));

		List<SigitVRicercaAllegatiDto> list = null;
		list = jdbcTemplate.query(sql.toString(), params, byIdAllegatoRowMapper);
